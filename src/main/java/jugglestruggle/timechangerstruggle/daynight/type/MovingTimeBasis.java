...
public abstract class MovingTimeBasis implements DayNightCycleBasis
{
    ...
    public void tick()
    {
        ...
        else
        {
            ...
            {
                var animationDelta = (double)this.ticksPassed / (double)this.ticksUntilNextCall;
                var result = this.easingBetweenTicks.value(this.easingType, animationDelta);

                this.previousCachedTime = this.cachedTime;

                this.cachedTime = (long)(this.previousInterpolatedTime + ((double)this.nextInterpolatedTime - (double)this.previousInterpolatedTime) * result);
            }
