	...
	private LiteralArgumentBuilder<FabricClientCommandSource> generateCycleSubcommand()
	{
		...
		(
			ctx -> 
			{
				...
				TimeChangerStruggleClient.getCachedCycleTypeBuilders().forEach(cycle -> 
				{
					...
					TextComponent useCycle = new TextComponent("[\u2192]");
					...
					useCycle.withStyle(style -> 
					{
						return style
							...
							.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, 
								new TranslatableComponent("jugglestruggle.tcs.cmd.cycle.listing.use", displayName)))
							.withBold(true);
					});
					...
					options.withStyle(style -> 
					{
						...
						if (cycle.hasOptionsToEdit())
						{
							currentStyle = currentStyle.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, 
								new TranslatableComponent("jugglestruggle.tcs.cmd.cycle.listing.option", displayName)))
					...
					MutableComponent displayNameAsDisplay = displayName.plainCopy();
					...
					Commands.sendTextToChat(ctx, new TranslatableComponent("%1$s %2$s %3$s", options, useCycle, displayNameAsDisplay));
				});
		...
		(
			...
			.executes(ctx -> 
			{
				...
				final TranslatableComponent cycleRemovedText = new TranslatableComponent
					("jugglestruggle.tcs.cmd.cycle.cycle", previousCycle.isPresent() ? previousCycle.get().getTranslatableName() : "??");
				final TranslatableComponent worldTimeText = new TranslatableComponent("jugglestruggle.tcs.screen.toggleworldtime");
...
//					}
	...
	private LiteralArgumentBuilder<FabricClientCommandSource> generateOptionSubcommand()
	{
		return ClientCommandManager.literal("option")
			...
			(
				"dateOverTicks", 
				new TranslatableComponent("jugglestruggle.tcs.screen.toggledate"), 
			...
			(
				"butterySmoothCycle", 
				new TranslatableComponent("jugglestruggle.tcs.screen.togglesmoothbutterdaylightcycle"), 
			...
			(
				...
				new TranslatableComponent("jugglestruggle.tcs.cmd.option.disablenightvision"), 
			...
			(
				...
				new TranslatableComponent("jugglestruggle.tcs.cmd.option.disableworldtimeoncycleusage"), 
			...
			(
				...
				new TranslatableComponent("jugglestruggle.tcs.cmd.option.commandfeedbackonlessimportant"), 
	...
	{
		final TranslatableComponent text = (args == null || args.length <= 0) ? 
			new TranslatableComponent(key) : new TranslatableComponent(key, args);
	...
	public static void sendTextToChat(CommandContext<FabricClientCommandSource> ctx, Component text) {
		ctx.getSource().getClient().gui.handleChat(ChatType.SYSTEM, text, Util.NIL_UUID);
	}
...
//				System.out.println("input cmd: "+starter);
				...
				TextComponent clickableText = new TextComponent(realCmd);
				...
				clickableText.withStyle(style -> {
					return style.withColor(0xDD44FF).withUnderlined(true).withBold(false)
						...
						.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TranslatableComponent(langCmd + ".hover")));
				});
	...
	private class CycleUseCommand implements Command<FabricClientCommandSource>
	{
		...
		public int run(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException
		{
			...
			{
				MutableComponent cycleText = new TranslatableComponent(langCmd+"cycle", 
					this.cycleToUse.getTranslatableName().plainCopy().withStyle(style -> style.withColor(0xFF22FF)));
			...
			{
				MutableComponent cycleText = new TranslatableComponent(langCmd+"cycle", 
					this.cycleToUse.getTranslatableName().plainCopy().withStyle(style -> style.withColor(0xFFFF22)));
		...
		public int run(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException
		{
			...
			if (TimeChangerStruggleClient.cachedCycleTypeExists("statictime")) 
			{
				if (!TimeChangerStruggleClient.isCycleTypeCurrentCycle("statictime")) 
				{
					...
					if (TimeChangerStruggleClient.commandsCommandFeedbackOnLessImportant)
					{
						...
						(
							...
							new TranslatableComponent("jugglestruggle.tcs.dnt.statictime")
						);
					}
				...
				if (TimeChangerStruggleClient.commandsCommandFeedbackOnLessImportant)
				{
					...
					TranslatableComponent myTimeTicks = new TranslatableComponent(langCmd + "ticks", timeTicks);
					TranslatableComponent totalTicks = new TranslatableComponent(langCmd + "ticks", totalTimeOfDay);
					TranslatableComponent prevTicks = new TranslatableComponent(langCmd + "ticks", previousTimeOfDay);
