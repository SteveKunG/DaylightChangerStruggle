...
public class EnumValue<EV extends Enum<EV>> extends BaseProperty<EnumValue<EV>, EV>
{
    ...
    public WidgetConfigInterface<EnumValue<EV>, EV> createConfigElement(TimeChangerScreen screen, FancySectionProperty owningSection)
    {
        ...
        {
            var sectionText = owningSection.get();

            if (sectionText != null && sectionText.getContents() instanceof TranslatableContents)
            {
                optionText = Component.translatable(String.format("%1$s.%2$s", ((TranslatableContents)sectionText.getContents()).getKey(), this.property().toLowerCase(Locale.ROOT)));
            }
        }
    ...
    public void readFromJson(JsonElement elem)
    {
        ...
        var valueName = prim.getAsString();
    ...
    public JsonElement writeToJson()
    {
        ...
        if (this.readableFunc != null)
        {
            var namedProp = this.readableFunc.applyRight(enumToWriteTo);
    ...
    static class EnumValueArgType<EV extends Enum<EV>> implements ArgumentType<EV>
    {
        ...
        {
            var beginningArg = reader.getCursor();
            ...
            var val = reader.getString().substring(beginningArg, reader.getCursor());
            var valLow = val.toLowerCase(Locale.ROOT);
        ...
        {
            final var startingText = builder.getRemainingLowerCase();
